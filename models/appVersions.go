...
var AppVersionWhere = struct {
	...
	ID:   whereHelperint{field: "\"appVersions\".\"id\""},
	Name: whereHelpernull_String{field: "\"appVersions\".\"name\""},
}
...
var (
	appVersionAllColumns            = []string{"id", "name"}
	appVersionColumnsWithoutDefault = []string{"name"}
	appVersionColumnsWithDefault    = []string{"id"}
	appVersionPrimaryKeyColumns     = []string{"id"}
)

type (
	// AppVersionSlice is an alias for a slice of pointers to AppVersion.
	// This should generally be used opposed to []AppVersion.
...
func (q appVersionQuery) One(ctx context.Context, exec boil.ContextExecutor) (*AppVersion, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "models: failed to execute a one query for appVersions")
	}
...
func (q appVersionQuery) All(ctx context.Context, exec boil.ContextExecutor) (AppVersionSlice, error) {
	...
	err := q.Bind(ctx, exec, &o)
	if err != nil {
		return nil, errors.Wrap(err, "models: failed to assign all query results to AppVersion slice")
	}
...
func (q appVersionQuery) Count(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to count appVersions rows")
	}
...
func (q appVersionQuery) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	...
	err := q.Query.QueryRowContext(ctx, exec).Scan(&count)
	if err != nil {
		return false, errors.Wrap(err, "models: failed to check if appVersions exists")
	}
...
func AppVersions(mods ...qm.QueryMod) appVersionQuery {
	mods = append(mods, qm.From("\"appVersions\""))
	return appVersionQuery{NewQuery(mods...)}
}

// FindAppVersion retrieves a single record by ID with an executor.
// If selectCols is empty Find will return all columns.
func FindAppVersion(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*AppVersion, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"appVersions\" where \"id\"=$1", sel,
	)
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "models: unable to select from appVersions")
	}
...
func (o *AppVersion) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	if o == nil {
		return errors.New("models: no appVersions provided for insertion")
	}
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"appVersions\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"appVersions\" %sDEFAULT VALUES%s"
		}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to insert into appVersions")
	}
...
func (o *AppVersion) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("models: unable to update appVersions, could not build whitelist")
		}

		cache.query = fmt.Sprintf("UPDATE \"appVersions\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 1, wl),
			strmangle.WhereClause("\"", "\"", len(wl)+1, appVersionPrimaryKeyColumns),
		)
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to update appVersions row")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by update for appVersions")
	}
...
func (q appVersionQuery) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to update all for appVersions")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to retrieve rows affected for appVersions")
	}
...
func (o AppVersionSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"appVersions\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, appVersionPrimaryKeyColumns, len(o)))
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to update all in appVersion slice")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to retrieve rows affected all in update all appVersion")
	}
	return rowsAff, nil
}
...
func (o *AppVersion) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
		return errors.New("models: no appVersions provided for upsert")
	}
	...
	if !cached {
		...
		if updateOnConflict && len(update) == 0 {
			return errors.New("models: unable to upsert appVersions, could not build update column list")
		}

		conflict := conflictColumns
		if len(conflict) == 0 {
			conflict = make([]string, len(appVersionPrimaryKeyColumns))
			copy(conflict, appVersionPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"appVersions\"", updateOnConflict, ret, update, conflict, insert)
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert appVersions")
	}
...
func (o *AppVersion) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if o == nil {
		return 0, errors.New("models: no AppVersion provided for delete")
	}
	...
	sql := "DELETE FROM \"appVersions\" WHERE \"id\"=$1"
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to delete from appVersions")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by delete for appVersions")
	}
...
func (q appVersionQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if q.Query == nil {
		return 0, errors.New("models: no appVersionQuery provided for delete all")
	}
	...
	result, err := q.Query.ExecContext(ctx, exec)
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to delete all from appVersions")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by deleteall for appVersions")
	}
...
func (o AppVersionSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"appVersions\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, appVersionPrimaryKeyColumns, len(o))
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to delete all from appVersion slice")
	}
...
func (o *AppVersion) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindAppVersion(ctx, exec, o.ID)
	if err != nil {
		return err
	}
...
func (o *AppVersionSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"appVersions\".* FROM \"appVersions\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, appVersionPrimaryKeyColumns, len(*o))
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to reload all in AppVersionSlice")
	}
...
func AppVersionExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"appVersions\" where \"id\"=$1 limit 1)"
	...
	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "models: unable to check if appVersions exists")
	}
